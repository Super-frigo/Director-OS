...
Version: 1.0.1
Status: Draft (updated for ADR-008)
...
- Compiler **不调用 LLM**——所有知识已在 Engine 阶段通过 Knowledge Resolution（ADR-008）解析完毕
...
Project (Structured Data)
    │
    ▼
Resolved Knowledge (from Knowledge Resolution, deterministic)
    │
    ▼
┌─────────────────────────────────────┐
│           Compiler Pipeline         │
...
```
相同 Project + 相同 Resolved Knowledge → 相同 Prompt（逐字节一致）
```

Compiler **永远不会调用 LLM**。所有 LLM 交互发生在 Engine 阶段的 Knowledge Resolution 层（ADR-008），其结果被缓存后以确定性形式传递给 Compiler。

从 Compiler 的视角，输入是完全确定的：
- Project（结构化数据，版本控制）
- Resolved Knowledge（来源可追溯，结果已缓存）

这保证了编译结果的可复现性、可审计性和可测试性。

---

# 9. Design Philosophy

These compiler principles derive from the canonical [ARCHITECTURE_PRINCIPLES.md](../architecture/ARCHITECTURE_PRINCIPLES.md).
...
5. 整个系统可扩展,不绑定任何单一平台
6. Compiler 保持确定性——不调用 LLM,所有知识从 Knowledge Resolution 获得（ADR-008）
